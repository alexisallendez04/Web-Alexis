# Brief — Estudio B&G

Estudio contable de Mar del Plata. Instagram: [@bygestudio](https://www.instagram.com/bygestudio/).

## Pendiente de cargar en `js/config.js`

| Campo | Dato |
|-------|------|
| WhatsApp | |
| Email | |
| Dirección exacta | Mar del Plata, Buenos Aires |
| URL | |

## Contenido ya volcado

- Quiénes somos (texto original, 2015)
- Valores: Gestión, Compromiso, Eficiencia, Responsabilidad, Desarrollo
- Servicios: Management, Finanzas, Impuestos, Contabilidad
- Paleta y tipografías del brief de marca
